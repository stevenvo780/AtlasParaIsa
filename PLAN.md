...
Los claros y arboledas de `7fff2f3` y la identidad de ejecución de `8565624` ya están integrados, aún sin publicar, con comprobaciones y revisión independiente. Se prepara una primera publicación V6 que los reúne con agua transportable y empieza otro mundo limpio. La medición CPU/CUDA está completada en su alcance ecológico: no justifica activar el puente GPU probado. La continuidad lejana sigue en una rama V7 con revisión propia del kernel; scheduler, archivo, promoción y experiencia requieren su aceptación conjunta. [EVIDENCIA](docs/EVIDENCIA.md) distingue cada resultado y no atribuye todavía un gate completo a la combinación final.
