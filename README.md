...
El candidato conjunto **`e42e8bc` sigue sin publicar**. Reúne la crónica completa y validación incremental del archivo de `4a05bb5`, la afinación familiar de `292f17a` y la lectura de etapas de vida. Permite crianza entre vecinos de comunidades distintas si ambos están afiliados y cumplen confianza, edad, cercanía, recursos y costes existentes; conserva sus comunidades. El [contraste local](docs/EVIDENCIA.md#confianza-entre-comunidades-contraste-local) acredita un nacimiento pagado, mientras su matriz larga y las semillas adicionales de contraste siguen en evaluación. Esa afinación no pertenece a la matriz de `189791d`, que conserva la física publicada.

La fuente **`409ee8f` pasó 612/612 Node, 18/18 pruebas de navegador compilado, tipos, build, smoke y 7/7 controles Python**. `e42e8bc` añade una línea CSS para ajustar etiquetas a 320 píxeles; su comprobación conjunta está en curso y aún no se le atribuye aquel resultado. Las comparaciones del archivo conservan el mundo exacto y reducen decodificaciones, sin demostrar una mejora estable del tiempo total. [Fuentes y límites](docs/EVIDENCIA.md#integridad-del-diagnóstico-y-coste-del-archivo). El servicio continúa en `03470e3`.

En ese candidato, el servidor determina las etapas «En crecimiento», «Edad de crianza» y «Vejez» con la edad y los umbrales heredados de cada vecino. El panel las cuenta globalmente, separadas de S/I protegidos, y declara los datos ausentes. La edad solo permite descartar o conservar posibilidad de recambio; no garantiza fecundidad ni un nacimiento. La ficha presenta la etapa como dato del modelo, sin modificar cuerpos ni almacenamiento. [Lectura y límites](docs/EXPERIENCIA.md#participación-de-isa).
...
La evidencia mínima de esa matriz se conservó también en almacenamiento privado del workspace: tres bases experimentales finales, los checkpoints 24000 y 36000 de la semilla fallida, auditorías, informes y scripts, con inventario de tamaños y SHA256 verificados. No contiene sesiones ni entradas y mantiene los originales. Omite otros checkpoints, logs de procesos y la captura privada comparativa: por sí sola no permite ejecutar íntegramente el analizador original. Los informes pequeños y sus sellos están enlazados desde EVIDENCIA; las bases permanecen privadas.
