...
Estado de la revisión: **V5 `6d0e53b` activo con mundo nuevo y acceso conservado**, con interfaz WebGL2/Canvas 2D/DOM, HTTP/WebSocket y SQLite. Tecnología, demografía y archivo de identidades tienen pruebas de integración y reemplazo generacional observado durante dos réplicas de veinticinco días; el catálogo finito y el mantenimiento material sin acreditar siguen limitando la autonomía. El candidato `61fb27d` añade checkpoints de apertura, probados y todavía sin desplegar. [README](../README.md) conserva operaciones y [EVIDENCIA](EVIDENCIA.md) resultados y archivo íntegro del V4 rechazado por validación. Los recuerdos son sintéticos; no hay alojamiento definitivo contratado.
...
Los recibos tecnológicos del servicio todavía son un búfer reciente de 256 entradas dentro del snapshot; no existe archivo persistente de procedimientos y ejecuciones fuera de él. Un actor sin recibos en la ventana puede conservar productos cuyo inventario inicial no se conoce. El adaptador no puede deducirlo del inventario final sin evidencia adicional. El candidato captura aperturas físicas completas dentro del snapshot existente, sin tabla SQL nueva; las publica junto al borrador confirmado y mantiene rollback y lectura del corte anterior. Su método y límites están en [REGLAS](REGLAS.md#inventarios-de-apertura-del-candidato). El observador experimental multisemilla acumula sus propios contadores completos; esa instrumentación no amplía las garantías del servicio.
