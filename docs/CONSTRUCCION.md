...
Estado de la revisión: **`03470e3` activa en la revisión privada para grabación, con SQLite 4 y reglas/protocolo 6**, publicada y comprobada el 6 de septiembre de 2026. Conserva aperturas de inventario, catálogo resoluble, caché de 256 definiciones, memoria local de 32 instrucciones, cuerpo compartido e índice ecológico en CPU. Añade al alcance de la publicación agua contenida, claros, identidad de ejecución y afinaciones de elección y lectura. El candidato conjunto sin publicar **`e42e8bc`** reúne journal completo, validación incremental del archivo, familias entre comunidades y etapas de vida derivadas. La fuente anterior `409ee8f` pasó 612/612 Node, 18/18 navegador compilado, tipos, build, smoke y 7/7 Python; el ajuste CSS de etiquetas de `e42e8bc` espera su comprobación conjunta. [README](../README.md) conserva operaciones y [EVIDENCIA](EVIDENCIA.md#matriz-v6-cerrada-a-veinticinco-días) separa publicación, integridad y matriz V6 cerrada: 30/18/1 vecinos finales, con la tercera semilla sin recambio posible. El contenido personal y el alojamiento definitivo siguen pendientes.

La integración inicial de agua **`95ff0d2`** mantuvo SQLite 4. Sus 120 fuentes, pruebas y archivos de compilación comparados coincidieron con `eefbbe2`, validado con 496/496 Node y 17/17 E2E. La fuente conjunta para grabación pasó 567/567 Node, 18/18 E2E compilados y smoke completo. [EVIDENCIA](EVIDENCIA.md#transporte-de-agua-v6-integrado) conserva el antecedente sin atribuirle los cambios ni la observación demográfica de la versión posterior.
...
El lector incorporado en `7d8777c` resuelve también la autoría de recetas fuera de RAM mediante el archivo de identidades. Una definición puede seguir apuntando a su inventor fallecido aunque su legado ya no esté en la caché; la consulta comprueba identidad y fechas sin volver a crear al habitante ni otorgar conocimiento a otros.
...
| Tecnología | Programas compuestos, ascendencia cultural, lotes con composición y propiedades, proyectos en curso, conocimiento y práctica locales. Caché y colas acotadas apuntan a definiciones, estadísticas y recibos durables. |
...
En el candidato, `PersonView.lifeStage?` admite `juvenile`, `adult` o `senescent`. `projectWorld` compara `p.demography.age` con la madurez y el comienzo de senescencia calculados por `demographicTraits(p.genome)`. Lo deriva para todos los habitantes actuales con independencia de la cámara, sin escribirlo en `World`, alterar RNG ni exponer nuevos alelos. No requiere migración ni cambio de SQLite 4 o protocolo 6. El cliente traduce la etapa recibida y declara su ausencia; no replica las fórmulas de edad. Censo y potencial por edad excluyen S/I del conjunto de vecinos. Los límites semánticos están en [REGLAS](REGLAS.md#salud-longevidad-y-cierre-de-una-vida) y la lectura en [EXPERIENCIA](EXPERIENCIA.md#participación-de-isa).

## Afinación de decisiones sin modificar las leyes corporales

`world/index.ts` compara acciones usando las operaciones existentes de `body.ts` y `demography.ts` sobre copias, sin consumir recursos durante el pronóstico. Descanso se pondera por recuperación efectiva; exposición usa el mismo predicado de techo que la demografía. El incentivo acotado por daño evitable conserva motivos de protección para S e I y excluye senescencia inevitable. Los coeficientes y límites del pronóstico están en [REGLAS](REGLAS.md#elección-según-recuperación-y-riesgo-físico).

La admisión de comida, agua y presa comparte rutas percibidas y presupuesto nominal con la evaluación de su beneficio. Una fuente inaccesible o una caza que el planificador descarta no apaga la búsqueda vital. Explorar sigue pagando movimiento; consumir exige débito real. `constructionOpportunity` y `repairOpportunity`, en `inventions.ts`, comparan servicio marginal local y costes, conservando ejecución y órdenes explícitas. Sus controles causales cortos se complementan con la matriz V6: el caso sin recambio no registró hambre ≥0,8 ni energía ≤0,2 entre vecinos después de los pasos. Eso orientó investigar condiciones familiares, sin identificar por sí solo una causa única de la trayectoria.

La afinación de `292f17a`, integrada en el candidato, retira la igualdad de comunidad tanto en `familyOpportunity` como en la comprobación de nacimiento. Ambos vecinos siguen necesitando afiliación, confianza mutua, aptitud corporal, recursos y proximidad bajo los límites existentes. Preparación y protección de la reserva consultan la misma oportunidad; no se concede alimento por declararla. Los progenitores conservan sus grupos y la cría adopta el del primero seleccionado. El [contraste local y sus controles](EVIDENCIA.md#confianza-entre-comunidades-contraste-local) verifican el nacimiento y sus costes; la matriz larga y las semillas adicionales siguen en evaluación. La modificación no cambia esquema, leyes corporales ni el servicio publicado.
...
Durante esta etapa, publicar una nueva versión de pruebas inicia otro mundo desde el paso cero, con base e historia anteriores resguardadas por separado y contraseña estable. La operación conserva la correspondencia entre cada archivo y su build, y separa los inventarios, regiones, identidades y recibos de cada ejecución. Las sesiones anteriores tampoco se heredan; se ingresa otra vez con la misma contraseña. Esta política de publicación no es un fallback del lector ni un reset del repositorio: los arranques ordinarios de una misma versión conservan su mundo. Se aplicó al publicar `7d8777c` y nuevamente en [la versión para grabación `03470e3`](EVIDENCIA.md#publicación-para-grabación); el procedimiento y la comprobación anterior están en [README](../README.md#nuevas-versiones-de-pruebas) y [EVIDENCIA](EVIDENCIA.md#publicación-con-mundo-nuevo).
...
El journal incorporado en `189791d` y conservado en el candidato validado `4a05bb5`, todavía sin publicar, separa la ventana visible de **120 hechos** de una cola durable de hasta **32768 hechos pendientes**. Un paso puede producir más de 120: cada emisión válida entra en ambas estructuras y se rechaza antes de modificar contadores si no cabe o contiene actores inválidos. Snapshot, cola completa y demás archivos se confirman juntos; el estado solo vacía pendientes después del commit. Un rollback conserva los hechos para reintentar. La matriz de tres semillas de 60000 pasos usó `189791d`, sin cambios físicos: auditó cada hecho, guardó cada 120 pasos y comprobó reinicio en 30000 y recarga final exactos. El candidato posterior se comparó también con esa fuente durante una continuación natural; ambas evidencias se separan en [EVIDENCIA](EVIDENCIA.md#integridad-del-diagnóstico-y-coste-del-archivo).
...
Un diario pendiente conserva todos los recibos nuevos antes de que el búfer reciente de 256 los descarte. La cola admite hasta **65536 recibos**; otra cola admite hasta **65536 definiciones nuevas o estadísticas modificadas**, independiente de la caché. Agotar cualquiera exige confirmar antes de seguir, sin descartar materia o historia. Snapshot, definiciones, estadísticas, recibos, regiones, legados, hechos y entradas se confirman en una transacción. Solo después de `COMMIT` se vacían las colas del estado publicado. Un rollback conserva sus pendientes para reintentar y no confirma órdenes; el snapshot propuesto no puede declarar confirmados registros ausentes del archivo.
...
### Contrato V6 de agua contenida

`material-affordances.ts` calcula capacidad, flujo, carga, derrame y fuga mediante cantidades enteras y resto fraccionario. `technology-water.ts` conecta esas operaciones con posesión, fuentes locales, fisiología, preparación autónoma y recibos. `technology-execution.ts` centraliza el registro tecnológico; fabricación, desgaste, transferencia, muerte y herencia conservan tanto la materia del soporte como su contenido líquido, con cuentas separadas. La interfaz recibe cantidades y propiedades proyectadas por el servidor: no ejecuta la física.

El estado opcional `contents` de un lote tiene versión 1, agua, resto de fuga y último paso procesado. El mundo V6 conserva un ledger de llenado, consumo, pérdida ambiental, trabajo y energía. Los recibos incluyen apertura y cierre líquidos, movimientos y fuente cuando corresponde; los checkpoints solo reconstruyen ventanas completas. El plan en curso persiste objeto, fuente, apertura, meta fija y comienzo, y continúa después de recargar sin cobrar dos veces ni desplazar la meta.

El lector valida V5 antes de migrar, rechaza campos de líquido bajo esa versión y añade cuentas vacías; no convierte el agua de la composición en contenido ni rellena recipientes. SQLite permanece en esquema 4: los cuerpos versionados del snapshot y del archivo tecnológico contienen estos campos. La integridad valida sus balances además de hashes; quitar el sobre de agua y recalcular una suma no hace válida una historia incoherente. Copias y builds V5 se conservan separados de estados V6. Esta ruta de compatibilidad no sustituye la política de comenzar un mundo nuevo al publicar.

## Conexión e interacción

El protocolo **V6** añade a cada producto un resumen opcional de agua: versión, cuantos contenidos, capacidad, escala de 50000 cuantos por unidad y fracción de fuga prevista por paso. Ausencia del resumen significa dato no disponible; cero significa vacío. El progreso de preparación usa la acción y el trabajo del habitante, con un motivo que distingue llenar de beber. Cliente y servidor compilados de la fuente para grabación comparten este contrato; el cliente rechaza una versión incompatible.

V6 conserva las ventanas de cámara con origen absoluto y órdenes individuales idempotentes introducidas antes, además de investigar, fabricar, cosechar alimento, reserva alimentaria visible, salud, vitalidad, productos, procedimientos y organización observada. La vista de un fallecimiento conserva información de identidad y retira los controles del cuerpo ausente. Ventanas distintas del mismo paso son válidas; una vista antigua no puede sustituir un paso posterior. El intercambio JSON incluye estado inicial, actualizaciones, solicitud de gesto u orden, resultado y error comprensible. Se envían vistas completas acotadas por cliente; no hay una simulación nueva asociada a cada conexión.

La vista publica por separado IDs de procedimientos recordados por cada habitante y el conjunto acotado de detalles de recetas disponible en esa proyección. La ficha distingue repertorio ausente, repertorio vacío y referencia cuyo detalle falta. El total mundial puede superar la lista del cuaderno; no se deduce memoria a partir de objetos portados ni de la caché visible. Esta separación no añade un explorador completo del archivo ni concede una receta al pulsar su referencia. La experiencia visible está descrita en [EXPERIENCIA](EXPERIENCIA.md#participación-de-isa).
...
**Una memoria causal archivada, con conocimiento local finito y producción guiada por necesidades observadas.** El catálogo activo desde `7d8777c` permite nuevas recetas después de llenar la caché, con resolución durable y aprendizaje local conectado. Reconstruir ventanas materiales archivadas y sostener reposición siguen pendientes. Las réplicas del cierre biológico V5 inicial no incluyen esta ampliación; la matriz V6 sí la incluye, pero no prueba mantenimiento material ni recambio en todas las semillas. La tabla distingue capacidades activas de trabajo todavía sin implementar; su integración no acredita cierre de la autonomía.
...
| 4. Caché y resolución — propietario de tecnología con contrato de archivo acordado | **Activo desde `7d8777c`:** 256 es el presupuesto de definiciones residentes; generación 32 deja de ser un freno histórico. Se resuelven técnicas archivadas conservando límites de operaciones, entradas, trabajo, enteros seguros y memoria. | Se superan 256 identidades históricas y generación 32 mediante procesos pagados con caché acotada; reinicio, copias y cortes anteriores resuelven exactamente sus dependencias. La amplitud del archivo requiere medición propia. |
| 5. Aprendizaje y redescubrimiento — propietario de sociedad y aprendizaje | **Activo desde `7d8777c`:** memoria de 32 instrucciones, recencia, olvido, protección de proyectos, enseñanza y redescubrimiento pagado sin acceso cognitivo global. La práctica se conserva solo con soporte local. Pendiente experimentar a partir de objetos desconocidos: `localInputs` aún exige conocer su receta; padres culturales y muestras materiales no son equivalentes. | Perder al último conocedor elimina fabricación sabida hasta enseñanza o redescubrimiento real; heredar o comprar un objeto no entrega su receta. Redescubrir un programa archivado conserva su ID y autoría, con otro recibo pagado. |
