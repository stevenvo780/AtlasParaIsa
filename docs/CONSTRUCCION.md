...
### Índice ecológico del candidato
...
Esta implementación mantiene fórmulas, orden y frecuencia del motor de referencia. Las pruebas de equivalencia no equivalen a una mejora de tiempo observada; la medición completa aislada está pendiente. Sigue ejecutándose en una CPU, sin trabajadores paralelos ni cálculo ecológico en GPU. Tampoco activa regiones lejanas ni cambia el presupuesto territorial.
...
### Archivo tecnológico del candidato
...
Un diario pendiente conserva todos los recibos nuevos antes de que el búfer reciente de 256 los descarte. La cola admite hasta **65536 recibos**; agotarla detiene el avance con un error explícito que exige confirmar, sin descartar materia o historia. Snapshot, cuatro tablas tecnológicas, regiones, legados, hechos y entradas se confirman en una transacción. Solo después del commit se vacían las colas del estado publicado. Un rollback conserva sus pendientes para reintentar y el snapshot propuesto no puede declarar confirmados recibos ausentes del archivo.
...
Este archivo no elimina todavía el máximo de 256 recetas residentes ni reconstruye un mundo mediante repetición física de recibos. El adaptador de organización sigue usando su ventana reciente y sus aperturas; consultar épocas completas en SQLite y obtener agregados comprobables son trabajos pendientes. Disco e índices crecen con la historia y necesitan medición independiente de la RAM.
...
`technology-organization.ts` adapta recibos materiales a `organization.ts`. Agrupa operaciones anidadas sin contar dos veces el desgaste, comprueba continuidad de inventarios y separa transferencias de producción. El análisis derivado se reutiliza entre proyecciones del mismo objeto, paso y contador de ejecuciones; no modifica la simulación. El candidato con checkpoints añade su referencia al criterio de reutilización: rotar una época invalida el informe anterior incluso sin cambiar paso o contador. Una ventana incompleta o un balance inconsistente se informa como evidencia insuficiente. Las condiciones de organización y sus límites científicos están en [CIENCIA.md](CIENCIA.md).

El servicio activo conserva solo 256 recibos recientes dentro del snapshot. Un actor sin recibos en la ventana puede mantener productos cuya apertura no se conoce; no se deduce del inventario final. El candidato captura aperturas físicas completas dentro del snapshot y las publica junto al borrador confirmado. La apertura no necesita una tabla adicional al archivo tecnológico descrito arriba. Su [método](REGLAS.md#inventarios-de-apertura-del-candidato) conserva desconocidos los huecos anteriores y distingue inventarios quietos exactos de transacciones agregadas por receta. El observador experimental multisemilla acumula sus propios contadores completos; esa instrumentación no amplía las garantías del servicio.
...
**Una memoria causal archivada, con conocimiento local finito y producción guiada por necesidades observadas.** Este tramo del objetivo rector debe permitir nuevas recetas después de llenar la caché y reconstruir ventanas de materiales sin aumentar indefinidamente la RAM. No arregla por sí solo demografía, división del trabajo ni mantenimiento. MAIN integra parcialmente los frentes 1, 2 y 3, todavía sin desplegar; el frente 5 dispone de funciones probadas sin conexión a la conducta. Las réplicas del servicio no incluyen estos cambios. La tabla conserva los siete frentes y sus partes pendientes.
...
|---|---|---|
| 1. Contratos tecnológicos — propietario de `shared/technology.ts` y `world/technology.ts` | **Parcialmente integrado:** contrato de definición inmutable —ID, firma, versión de leyes, programa, autor y padres— separado de estadísticas archivadas. El catálogo de ejecución sigue residente y limitado; falta resolver referencias tras retirar definiciones de RAM. | Evicción o recuperación no reutiliza IDs, cambia programas ni deja un producto apuntando a una receta distinta. |
| 2. Archivo transaccional — propietario de Store y snapshot | **Integrada la persistencia:** cuatro tablas, origen de cobertura, cola completa y límite explícito antes del commit. Pendientes agregados y presupuesto medido del archivo a largo plazo. Las transferencias mantienen sus recibos; archivar no prueba por sí solo balance físico. | Un fallo entre escrituras conserva la cola y el corte anterior; ningún snapshot confirmado depende de recibos todavía no persistidos. |
...
| 5. Aprendizaje y redescubrimiento — propietario de sociedad y aprendizaje | **Funciones disponibles, sin conectar:** recencia y olvido con capacidad finita, protección explícita de proyectos y rechazo atómico si no cabe un nuevo conocimiento. No enseñan por consultar el archivo. Pendientes causas de uso cotidiano y ensayos pagados con objetos desconocidos; hoy `localInputs` exige conocer su receta. Separar padres culturales de muestras materiales. | Perder al último conocedor elimina fabricación sabida hasta enseñanza o redescubrimiento real; heredar o comprar un objeto no entrega su receta. Un ensayo nuevo conserva identidad y procedencia propias. |
