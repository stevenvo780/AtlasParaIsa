...
Instrumento nuevo (`npm run techo -- --seed S --hilos N --gpu "…" --escala habitantes|teselas --hasta N --salida <dir>`) que hace crecer la escala del mundo hasta que el p95 del paso (ventana de `GOVERNOR_WINDOW_STEPS`=120, el MISMO `RollingStepPerformance` que usa el gobernador de producción, `src/server/governor.ts`) toca `gobernador.presupuestoMs`. Los sintéticos son clones estructurales de un fundador `neighbor` real (misma vía que `createWorld`, sin fábrica de personas paralela); igual que `scripts/lab/replica.ts`, adjunta SIEMPRE un `Store` temporal (con `persistencia.cadaTicks=1`, el default real de producción: «el paso» que mide el gobernador incluye clon + simulación + guardado). Tests: `tests/curva-techo.test.ts` (6/6), incluida la reproducibilidad de la secuencia de escalones entre dos corridas con la misma semilla (salvo tiempos) y que el criterio de parada real es cruzar el presupuesto, no agotar `--hasta`.

**Curva de HOY** (`--seed 51926 --hilos 1 --escala habitantes --hasta 8192`, SHA `694f6b6`, fichero `artifacts/lab/t109-curva-techo-20260922/curva.json`, no versionado — `artifacts/` está en `.gitignore`): **1 punto** — a 16 habitantes (la población de partida de `createWorld`, sin crecer todavía) el p95 YA toca el techo: `p50=29,19 ms`, `p95=63,12 ms` > presupuesto 50 ms, `tickHz=30,84`, `teselasActivas=2048`, `tesela/habitante=128`, `fraccionSerial=1` (no hay backend paralelo hoy, `--hilos`/`--gpu` se registran igual: `motor.hilos=1`, `motor.gpu=[]`). `motivoParada: "presupuesto"`. Corrida con `load average` de la torre en **39–45** sobre 32 hilos (varios worktrees hermanos de esta misma noche de sprint compitiendo por CPU: T103/T107/T108, barridos de laboratorio, `world.test.ts`, etc.) — el guardado con `Store` (dominante en el paso, ver nota de `scripts/lab/README.md#Rendimiento`) es sensible a esa contención, así que el techo medido aquí es el de **esta torre compartida en este momento**, no un límite del motor en sí.

**Corridas de validación con carga más liviana** (mismo instrumento y semilla, momentos previos de esta misma sesión, sin `--hasta` forzando el corte): la curva SÍ escala — 16 hab → `p50=12,49/24,34 ms`, `p95=28,55/48,85 ms`; 32 hab → `p50=20,59 ms`, `p95=36,97 ms`; 64 hab → `p50=28,2 ms`, `p95=50,06 ms` (techo). Confirma que el instrumento mide una curva real (crece con la población, no un artefacto fijo) y que su primer punto es del mismo orden de magnitud que el `p50=5,52 ms`/`p95=21,38 ms` medidos por `scripts/lab/replica.ts` para la misma semilla y escena de partida (16 habitantes, `DEFAULT_PARAMS`) — no idéntico porque las ventanas de medición difieren (120 pasos frescos desde el arranque aquí; 2400 pasos de un día completo, con el motor ya caliente, en `replica.ts`).

**Control** (regla 5, ficheros permitidos de T109 no tocan `src/world`): `digestoCanonico` intacto por construcción — `git diff --stat` contra `694f6b6` solo muestra `scripts/curva-techo.mts` (nuevo), `package.json`, `scripts/lab/README.md` y `tests/curva-techo.test.ts`; el `digest` sha256 de `src/world/*.ts` que el propio `curva.json` reporta (`bfeb6ae4f06e537e68c5e037f97fbc3bb35002720c5122a6ad6fb8ee7f78e534`) es el mismo de antes de esta tarea.

**Addendum — revisión adversarial (mismo día):** se verificaron con reproducción real los tres hallazgos y se corrigieron los dos reales:
1. **Confirmado y arreglado** — `tests/curva-techo.test.ts` solo comparaba el primer escalón entre dos corridas con la misma semilla. Se fortaleció para comparar TODOS los escalones que ambas corridas alcanzaron (no solo el 0), manteniendo la tolerancia solo en el número de escalones (el punto donde el p95 cruza `presupuestoMs` sí depende del ruido de esta torre compartida, verificado empíricamente: dos corridas del mismo comando en paralelo dieron 1 y 2 puntos; dos corridas secuenciales dieron 2 y 2 puntos idénticos salvo tiempos).
2. **Plausible y arreglado** — `agregarAnclasSinteticas` (escala teselas) usaba `cloneWorld`+`stepWorld` por ancla, avanzando `world.tick` sin medir (1 tick por ancla). Se sustituyó por una llamada directa a `maintainRegions(world, context)` (la misma función que `stepWorld` invoca internamente para activar/retirar chunks), que no toca `tick` ni corre ecología/fauna/demografía. Verificado: una corrida a `--escala teselas --hasta 65536` (semilla 51926) pasó de `tick` 120→242 (2 anclas, +2 ticks de calentamiento) ANTES del arreglo a `tick` 120→240 (+0 ticks de calentamiento) DESPUÉS, con el mismo par de puntos (2048/3840–3852 teselas) y sin ninguna violación de invariantes (`assertWorld`/`assertCommon` verdes en cada `store.save`).
3. **Confirmado y arreglado** — `tickHz` medía el lapso desde el ARRANQUE del primer paso hasta el ARRANQUE del último, omitiendo la duración propia de ese último paso (sesgo optimista, ~0,8 % en una ventana de 120 pasos de ~30 ms). Se corrigió a arranque-del-primero → FIN-del-último (arranque + duración), dividiendo por los 120 pasos completos en vez de 119 intervalos. La cifra `tickHz=30,84` de la «Curva de HOY» arriba quedó calculada con la fórmula VIEJA (sesgada al alza en ese ~0,8 %); no se re-mide esa curva puntual porque el sesgo es pequeño y la torre ya no reproduce la misma contención — el número queda como estaba, con esta nota, en vez de fabricar una repetición no comparable.

Verificación: `npm run typecheck` verde y `npx tsx --test --test-timeout=60000 tests/curva-techo.test.ts` 7/7 en 5 corridas consecutivas tras el arreglo.
