...
- **Ventana.** Con `persistencia.ventanaEventosTicks` > 0 cada guardado poda los recibos con tick < tick − max(ventana, 2 400): la misma ventana que ya poda sucesos y terreno (en producción 24 000 ticks = 10 días), nunca menos de un día, que es lo que leen detrás de cada guardado las métricas C7 del laboratorio (`durableActivityMetrics`; si un intervalo pedido empieza dentro de lo podado, fallan en voz alta en vez de contar de menos). No es un parámetro nuevo a propósito: `paramsOf` entra en `digestoCanonico`, y una clave nueva cambiaría el digesto de todos los mundos. Con ventana 0 (el default y el laboratorio) no se poda nada.
