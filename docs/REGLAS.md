...
- **Poco a poco.** Como mucho `TECHNOLOGY_PRUNE_ROWS_PER_SAVE` = 4 096 recibos por guardado (unas 9 veces lo que crece el mundo público por guardado con 75 habitantes, ≈ 450). Un mundo que ya existía se abre con el código nuevo sin intervención y se va podando: en la copia pública (351 486 recibos, 75 habitantes) convergió en 75 guardados (7 500 pasos, 12,5 min de servicio) a ~73 000 recibos retenidos (24 000 ticks × ~3 recibos por tick), con 37–92 ms de poda por guardado mientras migra y 7–16 ms después. Un tick con más recibos que el tope se poda entero y solo.
- **Verificable.** La poda comprueba que el tramo está completo, en orden y que cada cuerpo sigue siendo el de su digesto, y guarda en `metadata` la frontera `technology-pruned-v1` = {serie, tick del último podado, cuántos, pliegue `sha256(anterior + "\n" + digesto)` de los digestos de fila desde el origen}. Cada guardado escribe además, en la misma transacción que la instantánea, la cabeza de esa cadena (`technology-chain-v1`). `load()` verifica la cobertura desde la frontera —como `assertChronicleArchive` desde `chronicle-pruned-v1`— y comprueba que frontera + filas retenidas cierran la cabeza: una fila retenida borrada, reordenada o reescrita (aunque se recalcule su digesto), una frontera adelantada o con otro digesto, o una frontera sin cabeza, fallan cerradas (`tests/technology-retention.test.ts`). Un archivo anterior a la retención no tiene cabeza: la primera carga la calcula y el primer guardado la escribe; una cabeza atrasada (la dejaría un binario anterior que siguió guardando sin mantenerla) se comprueba como prefijo. `previous()` cierra la cabeza del original sobre lo retenido antes de borrar nada y la copia recuperada empieza con la suya.
