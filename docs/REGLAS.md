...
brazo «paquete sin memoria del agua» frente a control con los params históricos, 16 semillas cada uno;
recuento final con las réplicas relanzadas tras el incidente de disco): al **día 10**, 15 de 16 semillas
(**94 %**) sostienen ≥ 16 habitantes con **mediana 63**, frente a 14 de 16 (88 %) y **mediana 24,5** del
control; en los 16 pares el paquete queda por encima en 14 (7: 92 frente a 18; 42: 42 / 16; 3: 156 / 51;
1234: 106 / 25; 51926: 102 / 50; 777: 115 / 48; …) y por debajo en 2 (1: 63 / 64; 20260919: 8 / 13).
Hacia el día 30 el control deja solo a S e I en 10 de 16 semillas; el paquete, en 1 (20260919). Con las reglas históricas la semilla 7 no pare a nadie en 3
días; con los defaults nuevos sí (`tests/reglas10-defaults.test.ts`). **Lo que esto no acredita:** al día
15 el paquete da 9 de 10 réplicas con dato ≥ 16 (mediana 68) frente a 7 de 15 del control (mediana 13),
...
La refutación está en `tests/params-instantanea.test.ts`: un mundo generado con `agua.cuencas=1` tiene el agua potable de ese régimen (no la del default), un mundo guardado con `agua.cuencas=0.8,poblacion.maxima=50` los conserva al recargar con el mismo dígeste, la receta del despliegue conserva del mundo cargado toda clave que no nombra mientras impone la cadencia, y la instantánea sin campo vuelve a los params históricos (`HISTORICAL_PARAMS`, que hasta reglas 10 coincidían con `DEFAULT_PARAMS`).
...
| `persistencia.ventanaEventosTicks` | 0 (sin poda; producción 24 000 = 10 días vía `PRODUCTION_PARAMS`) | [0, 1000000] | Ventana de retención temporal para la poda de eventos, chunks y recibos de ejecución (los recibos, nunca a menos de un día; ver «Retención del archivo de recibos») | T021 · noche-arch 2026-09-23 |
...
**Retención del archivo de recibos (FR-016, sprint noche-arch 2026-09-23).** `technology_executions` guarda el recibo inmutable de cada ejecución material (investigar, fabricar, usar, reciclar, agua…) y era el 92 % del mundo público: 1 103 MiB de 1 199 con 351 486 recibos a día 66, creciendo con la edad y con la población, y `load()` lo verificaba entero —23–81 s, el 83–89 % de la carga—. El mundo **no lee** esos recibos: el paso resuelve definiciones y estadísticas (`catalogueReader`), la interfaz usa el anillo residente de 256 (`state.history`) y las definiciones, y la crónica no los cita. Por eso retenerlos por ventana no cambia la simulación: `digestoCanonico` es idéntico con y sin la poda (abajo). La ley:

- **Ventana.** Con `persistencia.ventanaEventosTicks` > 0 cada guardado poda los recibos con tick < tick − max(ventana, 2 400): la misma ventana que ya poda sucesos y terreno (en producción 24 000 ticks = 10 días), nunca menos de un día, que es lo que leen detrás de cada guardado las métricas C7 del laboratorio (`durableActivityMetrics`, `roleActivity`; si un intervalo pedido empieza dentro de lo podado, fallan en voz alta en vez de contar de menos). No es un parámetro nuevo a propósito: `paramsOf` entra en `digestoCanonico`, y una clave nueva cambiaría el digesto de todos los mundos. Con ventana 0 (el default y el laboratorio) no se poda nada.
- **Lo que nunca se poda.** Nada que necesiten las tres instantáneas que quedan tras el guardado (el candidato y lo que la rotación deje en los slots 1 y 2): ni sus recibos confirmados (la cobertura de `load()` en caliente y de `previous()`) ni ninguno desde el primero de su anillo residente, que se compara con el archivo. Cada respaldo se mira —se decodifica una vez por proceso el que no se escribió ni se cargó—, no se supone su edad: con reinicios frecuentes el slot 2 envejece y frena la poda. Se corta siempre en un tick completo (un padre y sus ejecuciones anidadas comparten tick).
- **Poco a poco.** Como mucho `TECHNOLOGY_PRUNE_ROWS_PER_SAVE` = 4 096 recibos por guardado (unas 9 veces lo que crece el mundo público por guardado con 75 habitantes, ≈ 450). El corte es por ticks completos: se elige el último tick que cabe entero en el tope. Un único tick con más recibos que el tope se poda entero y solo en su guardado (no se parte un padre de sus anidadas y la poda no se atasca en él; su coste es el de lo que un paso escribió en un guardado). Un mundo que ya existía se abre con el código nuevo sin intervención y se va podando: en la copia pública (351 486 recibos, 75 habitantes) convergió en 75 guardados (7 500 pasos, 12,5 min de servicio) a ~73 000 recibos retenidos (24 000 ticks × ~3 recibos por tick), con 37–92 ms de poda por guardado mientras migra y 7–16 ms después.
- **Verificable.** El formato V2 comprueba que el tramo está completo, en orden y que cada cuerpo sigue siendo el de su digesto. La cadena pliega cada fila como `sha256(anterior + "\n" + digesto + "\n" + tick)`. La frontera `technology-pruned-v1` (se conserva la clave histórica, pero su valor declara `version: 2`) guarda serie, tick del último podado, total, conteos canónicos de los siete tipos, cabeza del tramo y un sello SHA-256 sobre todos esos campos. Cada guardado escribe además, en la misma transacción que la instantánea, la cabeza V2 (`technology-chain-v1`, también clave histórica) y repite allí el sello de la frontera: cambiar cualquier campo rompe ese enlace además de la cadena de recibos. `load()` verifica el sello, exige que el primer recibo retenido tenga un tick estrictamente posterior a la frontera, que la cabeza V2 sea exactamente la de la instantánea (nunca una atrasada) y que frontera + filas retenidas la cierren: cambiar tick, total o conteos por tipo sin rehacer el sello, borrar, reordenar o reescribir una fila (aunque se recalcule su digesto), adelantar la frontera, retroceder o retirar la cabeza falla cerrado (`tests/technology-retention.test.ts`). Alcance: el sello es un SHA-256 sin clave y vive en la misma base, así que detecta corrupción, escrituras a medias y ediciones incoherentes, no a quien reescribe a propósito frontera, sello y cabeza a la vez (podría igualmente reescribir las instantáneas); impedirlo exigiría una clave fuera de la base, que el servidor no tiene. Un archivo anterior a la retención sin poda puede traer una cabeza V1: se verifica, también atrasada (como prefijo), y el siguiente guardado la actualiza a V2. Una base ya podada con el formato V1 no tiene los datos necesarios para construir el sello ni los conteos por tipo porque las filas desaparecieron; por eso falla cerrada con `legacy prune boundary has no authenticated V2 seal` y exige recuperación explícita desde un respaldo anterior a la poda. No hay bases podadas V1 en producción. `previous()` cierra la cabeza del original sobre lo retenido antes de borrar nada y la copia recuperada empieza con la suya.
- **Lo que no se mueve.** `journal.startsAfter` vive en el mundo (instantánea y digesto) y `technology_origin` es inmutable, así que la frontera va en metadatos del anfitrión, como la de la crónica, y no «mueve el origen lógico del journal» como decía el primer borrador de FR-016. Una ejecución anidada ausente es una referencia conservada si su serie quedó antes del origen **o dentro del prefijo podado** (el archivo pregunta la frontera al anfitrión en cada verificación, nunca la recuerda).
- **Compatibilidad.** Tras la primera poda un binario anterior ya no abre la base (su cobertura empieza en el origen y encuentra el hueco): volver atrás exige un respaldo previo. Los respaldos horarios anteriores a la poda conservan los recibos viejos; nada del código los necesita.

**El sello deja de caer por lo que no es del archivo.** `save()` reutiliza la verificación de la carga mientras el sello de la base (`data_version`, `total_changes()`, esquema) no se mueva. `addSession` y `revoke` escribían por la misma conexión y lo movían: cada inicio de sesión de Isa hacía que el guardado siguiente releyera la instantánea y recorriera el archivo de recibos **dos veces** (línea base y candidato). Ahora las sesiones se escriben en su propia transacción y, si el sello avanzó exactamente lo que cambiaron (sin disparadores, sin otra conexión en medio y sin cambio de esquema), las pruebas avanzan con él; un guardado rechazado por sesión revocada deshace una transacción vacía y tampoco las tira. Cuando el sello sí cae (una escritura ajena, `npm run access`), la línea base se barre una vez y del candidato —que por contrato tiene el mismo intervalo— sólo se compara su anillo. Y la verificación por fila cuesta un tercio: cronología contra la fila vecina de la misma página en lugar de releer y decodificar las dos vecinas de cada fila, origen y frontera una vez por página, anidadas tomadas de la página, expresiones regulares precompiladas (3,11 → 1,00 decodificaciones por fila). Una base nueva usa páginas de 8 KiB (un recibo ronda 2 KB y en una hoja de 4 KiB cabe uno solo); una existente conserva las suyas.

Medido en copias de la copia pública de las 04:00 (tick 158 600, 75 habitantes, 351 486 recibos; torre cargada, carga media 23–75, `nice 10`; evidencia en `docs/evidencia-2026-09-23-retencion/`):

| Medida | Base `715958f` | Esta ley |
|---|---|---|
| `load()` con el archivo entero (351 486 recibos), dos rondas en paralelo | 50,5–92,6 s (48–64 s CPU) | 31,1–40,1 s (29–32 s CPU) |
| `load()` tras la migración (74 941 recibos), mismas rondas | — (la base ya no abre un archivo podado) | 16,6–21,3 s (11,2–12,4 s CPU); con la torre más tranquila, 7,8 s |
| Primer guardado tras cargar | 0,26–0,90 s | 0,63–1,37 s (poda 4 096 recibos y decodifica los slots 1 y 2) |
| Guardado tras un inicio de sesión | 64–145 s (74–123 s CPU) | 0,34–1,07 s |
| Guardados siguientes (cadencia 100), mediana durante / después de migrar | 0,99 / 1,04 s (0,58 / 0,63 s CPU) | 0,77 / 1,09 s (0,63 / 0,62 s CPU) |
| `technology_executions` (dbstat) | 1 103 MiB | 209 MiB |
| Respaldo `VACUUM INTO` / comprimido como `respaldo.sh` | 1 199 / 79,7 MiB | 299 / 31,6 MiB (253 / 30,3 MiB con páginas de 8 KiB) |

El fichero vivo **no encoge** (1 199 MiB en los dos casos): SQLite deja las 230 070 páginas liberadas (899 MiB) en su lista libre y las reutiliza para lo que llega, así que deja de crecer mientras la lista no se agote; un `VACUUM` con el servicio parado (al publicar) lo baja a los ~300 MiB del respaldo. Con la retención convergida la carga ya no la domina el archivo de recibos (1,8 s de 7,8) sino `summarizeDefinitions` (3,7 s: recorre las 19 830 definiciones y 53 948 filas de estadísticas, que no se podan) y la migración del mundo (1,0 s).

Identidad: `digestoCanonico` idéntico en la copia pública avanzada con el régimen de `main.ts` —12 de 12 guardados en 1 200 pasos y 90 de 90 en 9 000 pasos, que incluyen la migración entera— y en las semillas de control 51926 y 7 (leyes candidatas) y 42 (históricas), 4 800 pasos con Store, sin ventana y con ventana 2 400 (la rama podó 2 414, 2 450 y 1 780 recibos respectivamente; la base, ninguno): base `715958f` frente a esta ley, `scripts/perf/identidad-retencion.ts` y `scripts/perf/retencion-publico.ts`.
