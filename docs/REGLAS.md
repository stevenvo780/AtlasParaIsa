...
- **Verificable.** El formato V2 comprueba que el tramo está completo, en orden y que cada cuerpo sigue siendo el de su digesto. La cadena pliega cada fila como `sha256(anterior + "\n" + digesto + "\n" + tick)`. La frontera `technology-pruned-v1` (se conserva la clave histórica, pero su valor declara `version: 2`) guarda serie, tick del último podado, total, conteos canónicos de los siete tipos, cabeza del tramo y un sello SHA-256 sobre todos esos campos. Cada guardado escribe además, en la misma transacción que la instantánea, la cabeza V2 (`technology-chain-v1`, también clave histórica) y repite allí el sello de la frontera: cambiar cualquier campo rompe ese enlace además de la cadena de recibos. `load()` verifica el sello, exige que el primer recibo retenido tenga un tick estrictamente posterior a la frontera y comprueba que frontera + filas retenidas cierran la cabeza: falsificar tick, total o conteos por tipo, borrar, reordenar o reescribir una fila (aunque se recalcule su digesto), adelantar la frontera o retirar la cabeza falla cerrado (`tests/technology-retention.test.ts`). Un archivo anterior a la retención sin poda puede traer una cabeza V1: se verifica y el siguiente guardado la actualiza a V2. Una base ya podada con el formato V1 no tiene los datos necesarios para construir el sello ni los conteos por tipo porque las filas desaparecieron; por eso falla cerrada con `legacy prune boundary has no authenticated V2 seal` y exige recuperación explícita desde un respaldo anterior a la poda. No hay bases podadas V1 en producción. `previous()` cierra la cabeza del original sobre lo retenido antes de borrar nada y la copia recuperada empieza con la suya.
