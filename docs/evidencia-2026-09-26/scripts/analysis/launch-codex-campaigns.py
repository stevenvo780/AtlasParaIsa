...
def main() -> int:
    ...
    if profile == "tower" and not os.sched_getaffinity(0).issubset(set(range(10, 32))):
        raise SystemExit("El gestor de la torre debe iniciarse dentro de CPUs 10-31")
    ...
    def watchdog() -> None:
        ...
        while not stop.wait(30):
            ...
            if free < 20:
                ...
                return
            if profile == "tower" and available_gib() < 20:
                halt("Torre con menos de 20 GiB disponibles")
                return
            if healthy():
                unhealthy_since = None
            else:
    ...
    def run_one(job) -> None:
        ...
        env = os.environ.copy()
        env["TMPDIR"] = str(tmp)
        command = ["npx", "tsx", "scripts/lab/replica.ts", "--seed", str(seed),
                   "--dias", "60", "--params", params, "--salida", str(output_dir)]
        # renice establece prioridad absoluta 19 incluso si ananicy cambió la del gestor.
        cmd = ["bash", "-c", 'renice -n 19 -p "$$" >/dev/null && exec "$@"',
               "replica-job", *command]
        if profile == "tower":
            cmd = ["taskset", "-c", "10-31", *cmd]
        with log_path.open("w") as log:
            # El watchdog y los lanzamientos comparten el mismo candado: un hijo
            # nunca aparece después de que halt() haya tomado su inventario.
            with write_lock:
                if stop.is_set():
                    child = None
                else:
