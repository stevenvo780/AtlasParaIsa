...
1. **Teselas de chunks inactivos.** La implementación contaba 0 para toda tesela fuera de los chunks activos (el motor activa ±8 celdas alrededor de cada persona), así que x_L dependía de la rejilla de chunks y de personas a más de radioLugar + R (una persona a 25 celdas invertía la decisión en la huerta fundadora de la semilla 42), y los discos de los lugares fundadores quedaban recortados al arrancar. Corrección: una tesela de un chunk inactivo se evalúa con su estado de GENERACIÓN (`initializeEcosystem(generateTile(semilla, x, y, cuencas))`, la misma composición con que el motor materializa un chunk nuevo; función pura, en caché). Consecuencia declarada: un chunk dormido ya visitado cuenta con su estado inicial, no con su historia, y sus cisternas no cuentan. Leer el estado congelado dependería de la cadencia de guardado (`Store.save` vacía `retiredChunks`).
