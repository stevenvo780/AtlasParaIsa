...
try {
  ...
  const result = await page.evaluate(async ({animated}) => {
    ...
    const world: WorldView = { version: 3, sequence: 1, tick: 1, day: 1, phase: 'day', weather: 'clear', width: 88, height: 56, originX: 0, originY: 0, tiles, people, places: [], events: [], memories: [], infinite: true };
    ...
    const move = () => { movingWorld = {...movingWorld, tick: movingWorld.tick+1, sequence: movingWorld.sequence+1, people: movingWorld.people.map(p => ({...p,x:p.x+.3,y:p.y+.1}))}; landscape.update(movingWorld); };
    if (animated) move();
