/** Reserved paired audit. `prepare` never starts simulations; `run` is explicit.
...
const BASELINE = '9665754c15e9c9c40b025a69ccf990896bc3849c';
const DAYS = 30, TICKS = DAYS * 2400, WORKERS = 24, TIMEOUT_MS = 3_600_000;
const INSTRUMENT_FILES = ['scripts/lab/coherence.ts', 'scripts/lab/metrics.ts'];
...
function insideReserved(path: string): void {
  ...
  if (!rel || rel.startsWith('..') || resolve(RESERVED, rel) !== path) throw new Error('Batch directories must be children of artifacts/coherence-20260922/reserved-long.');
}
...
export function prepare(directory: string): Batch {
  ...
  try {
    ...
    if (!/export const RULES_VERSION = 7;/.test(readFileSync(join(candidateRoot, 'src/world/index.ts'), 'utf8'))) throw new Error('Candidate must declare rules V7.');
    ...
    writeFileSync(join(directory, 'coherence-batch.ts'), readFileSync(fileURLToPath(import.meta.url)), { flag: 'wx', mode: 0o444 });
    for (const folder of ['runs', 'logs', 'results', 'tmp']) mkdirSync(join(directory, folder));
    const jobs = Array.from({ length: 32 }, (_, n) => 1000 + n).flatMap(seed => (['baseline', 'candidate'] as const).map(side => ({
    ...
    const batch: Batch = { version: 1, createdAt: new Date().toISOString(), directory, sourceWorkspace: ROOT,
      ...
      scope: '32 paired reserved seeds 1000..1031, 30 days. stepWorld and production SQLite, no governor, scheduler, clients or network. Concurrent timings are not a server performance gate.' };
...
function verifyPlan(batch: Batch): void {
  ...
  if (batch.version !== 1 || batch.days !== DAYS || batch.ticks !== TICKS || batch.workers < 1 || batch.workers > WORKERS || !Number.isInteger(batch.workers)
    || batch.timeoutMs !== TIMEOUT_MS || batch.engine !== 'world' || batch.params !== 'persistencia.cadaTicks=20' || batch.metricasVersion !== 2 || batch.jobs.length !== 64) throw new Error('Unsupported batch contract.');
  const expected = new Set(Array.from({ length: 32 }, (_, n) => 1000 + n).flatMap(seed => [`baseline-${seed}`, `candidate-${seed}`]));
...
export async function run(batch: Batch): Promise<Result[]> {
  ...
  async function launch(job: Job): Promise<Result> {
    ...
    try {
      child = spawn(process.execPath, ['--import', pathToFileURL(join(batch.sourceWorkspace, 'node_modules/tsx/dist/loader.mjs')).href, join(batch.instrumentRoot, 'scripts/lab/coherence.ts'),
    ...
    return await new Promise(resolveResult => {
      ...
      child.once('close', (code, signal) => {
        clearTimeout(timer); clearTimeout(killTimer); active.delete(child);
...
async function main() {
  ...
  if (extra.length || !['prepare', 'run', 'check'].includes(mode ?? '') || (mode !== 'prepare' && !directory)) throw new Error('Usage: tsx scripts/lab/coherence-batch.ts prepare [new-directory] | check batch-directory | run batch-directory');
  if (mode === 'prepare') {
    const batch = prepare(directory ?? join(RESERVED, `batch-${stamp()}`));
    console.log(JSON.stringify({ state: 'prepared', directory: batch.directory, simulationsStarted: 0, jobs: batch.jobs.length, ticks: TICKS * 64, workers: WORKERS, timeoutSeconds: TIMEOUT_MS / 1000 }));
  } else {
