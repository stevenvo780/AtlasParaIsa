...
import { FamilyObservation, familySample, roleActivity } from './family-observation.js';
...
const instrumentHash = createHash('sha256').update(readFileSync(fileURLToPath(import.meta.url)))
  .update(readFileSync(fileURLToPath(new URL('./metrics.ts', import.meta.url))))
  .update(readFileSync(fileURLToPath(new URL('./family-observation.ts', import.meta.url)))).digest('hex');
...
const observation = new FamilyObservation();
...
try {
  ...
  for (let day = 1; day <= days; day++) {
    ...
    for (let tick = 0; tick < api.TICKS_PER_DAY; tick++) {
      ...
      observation.end(world);
      if (world.tick % 120 === 0) appendFileSync(join(output, 'family-samples.jsonl'), JSON.stringify(familySample(world, family)) + '\n');
    ...
    const record = { day, tick: world.tick, population: world.people.length, births: world.totals.births,
      ...
      familyObservation: observation.closeInterval(),
      usefulActivityByRole: roleActivity(world, store.db, lastTick),
