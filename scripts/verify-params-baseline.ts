/** T102: compara estado físico completo con una copia del commit anterior.
 * node --import tsx scripts/verify-params-baseline.ts BASELINE_DIRECTORY [ticks=1200]
...
assert.deepEqual(changed, ['shared/param-syntax.ts', 'world/params.ts'], 'el control T102 no admite cambios físicos');
const directory = mkdtempSync(join(tmpdir(), 'atlas-t102-parity-')), results: object[] = [];
...
try {
  for (const seed of [1, 51926, 20260905]) for (const options of ['', reserved]) {
    ...
    try {
      ...
      results.push({ seed, mode: options ? 'reserved' : 'defaults', ticks, physicalSha256: hash(physicalBytes(b)), baselineDigest, candidateDigest });
