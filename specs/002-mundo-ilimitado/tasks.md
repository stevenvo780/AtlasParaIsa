...
- [ ] **T101** [claude opus · high] **Digesto canónico** — la puerta de calidad de toda la feature.
  Nuevo `digestoCanonico(world): string` (sha256) con **orden total explícito**: teselas por `(x,y)`, personas por slot, animales/estructuras/lugares por su clave canónica, **claves de todo `Record` ordenadas** (`bonds`, `skills`, `activity`, `values`, `totals`, `chunks`), **`retiredChunks` INCLUIDO** (ordenado por `key`), `chronicleJournal` excluido. Además `diferenciaCanonica(a, b)` que localiza el **primer** desacuerdo e informa sección/índice/campo. No toca `encodeSnapshot`: persistir y verificar son trabajos distintos.
...
- [ ] **T102** [claude sonnet · high] **Parámetros de motor y reversión por etapa.**
