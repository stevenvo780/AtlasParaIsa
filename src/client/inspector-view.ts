...
export function inheritedAndLearned(p: PersonView, world: WorldView, recipeIds?: string[], extra?: Pick<PersonDetail, 'fertil' | 'busca' | 'familia' | 'edades'> | null, buscarEpisodio: (id: string) => ChronicleEvent | undefined = id => world.events.find(event => event.id === id)): { now: string; kit: string; story: string } {
  ...
  const experiences = `<details class="person-detail" data-detail="experiences"><summary>Últimas experiencias${p.experiences === undefined ? '' : ` <span class="detail-badge">${Math.min(8, p.experiences.length)}</span>`}</summary><p>Registro reciente de esta vida simulada. Conserva el momento y el episodio que lo originó.</p>${p.experiences === undefined ? '<p class="drawer-note" data-loading="experiences">Cargando sus últimas experiencias…</p>' : p.experiences.length ? `<ol class="experience-list">${p.experiences.slice(-8).reverse().map(experience => { const cause = buscarEpisodio(experience.causeId); return `<li><span class="experience-tick">${esc(momento(experience.tick).toLocaleUpperCase('es'))}</span><p>${esc(enClaro(experience.text, { world }))}</p><small><strong>Qué influyó:</strong> ${esc(cause ? enClaro(cause.cause, { world }) : 'El episodio que lo causó no está entre los que recibió este navegador.')}</small></li>`; }).join('')}</ol>` : '<p>Aún no hay experiencias registradas.</p>'}</details>`;
