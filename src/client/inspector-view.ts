...
export function inheritedAndLearned(p: PersonView, world: WorldView): { now: string; kit: string; story: string } {
  ...
  const toolkit = products.length ? `<details class="person-detail" data-detail="products"><summary>Objetos que lleva <span class="detail-badge">${products.length}</span></summary>${products.slice(0,8).map(item=>`<div class="skill-row"><span>${item.recipeId ? procedureReference(item.recipeId, technology) : 'Material sin procedimiento asociado'}</span><strong>${number(item.mass/1000,2)} u.</strong></div>`).join('')}<p>Objetos fabricados que conserva este habitante. Su masa cambia al usarlos o transformarlos. Llevar un objeto no implica recordar cómo se fabrica.</p></details>` : '';
