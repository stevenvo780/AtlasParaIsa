...
export function origenReceta(detail: Pick<TechnologyRecipe, 'inventorId' | 'tick'>): string {
  const autor = nombreConocido(detail.inventorId);
  return `Inventada ${autor ? `por ${autor}` : 'por un habitante que este navegador no ha visto'} el día ${n(Math.floor(detail.tick / 2400) + 1, 0)}.`;
}
