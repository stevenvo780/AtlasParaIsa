...
import type { RuntimeStats, WorldStats } from '../shared/types.js';
...
/** Nota de «Diversidad de oficios»: qué mide (entropía de los oficios dominantes sobre los 18 posibles,
 * diversidad.ts) y, solo si el mundo mostrado aplica `conducta.habituacion` > 0, qué hace esa ley: resta
 * valor a repetir lo ya practicado, más con más curiosidad (world/index.ts). Sin el dato no se afirma. */
export function notaDiversidadOficios(stats: WorldStats | undefined, conducta: RuntimeStats['conducta']): string {
  const habituacion = conducta && Number.isFinite(conducta.habituacion) && conducta.habituacion > 0
    ? ' · Con habituación: lo que alguien ya practicó mucho le atrae menos, y más cuanto más curioso es' : '';
  const paso = stats?.statsTick !== undefined ? ` · medido en el paso ${n(stats.statsTick, 0)}` : '';
  return `Reparto de los oficios dominantes (100 % = por igual entre los 18 posibles)${habituacion}${paso}`;
}

export function technologyPane(technology?: TechnologyView, organization?: OrganizationSummary,
  details?: ReadonlyMap<string, TechnologyRecipe | null>, stats?: WorldStats, conducta?: RuntimeStats['conducta']): string {
