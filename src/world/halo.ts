...
export const ALCANCES: readonly Alcance[] = [
  ...
  a('decision.personas', 'personas', 'entrada', 7, 'index.ts', 'choose', ['const nearbyPeople = world.people.filter(other => other.id !== person.id && distance(person, other) <= RADIUS);']),
  ...
  a('decision.techo', 'estructuras', 'entrada', 0, 'index.ts', 'bodilyShelter',
    ["tileAt(world, point)?.terrain === 'shelter' ? Math.max(0, ...world.structures", ".filter(s => s.x === point.x && s.y === point.y && s.condition > BROKEN_CONDITION && s.components.includes('roof'))", '.map(s => s.condition))']),
  ...
  a('decision.companeros', 'personas', 'entrada', 7, 'index.ts', 'explorationTarget', ['world.people.filter(p => p.id !== person.id && distance(person, p) <= 7 && (person.bonds[p.id] ?? 0) > 0.3)']),
  a('familia.pareja', 'personas', 'entrada', 7, 'family.ts', 'familyOpportunity', ['const partner = world.people.filter(other => other !== person && other.id !== person.id && !!other.communityId',
  ...
  ...(['asentamiento.hogar', 'asentamiento.lugares'] as const).flatMap(centro => [
    ...
    a(`${centro}.personas`, 'personas', centro, 6, 'society.ts', 'settlementOpportunity', ['const peers = world.people.filter(p=>p!==person && distance(p,place)<=6);']),
  ]),
  // Cooperación: el otro a ≤ 7, y desde él su destino y sus insumos.
  a('cooperacion.otro', 'personas', 'entrada', 7, 'society.ts', 'evaluateCooperation', ['for (const other of world.people) {', 'if (other === person || distance(person, other) > 7 || world.tick - person.lastSocial < 30) continue;']),
  ...
  a('obra.servicios', 'estructuras', 'entrada', 7, 'inventions.ts', 'localServices', ['for (const structure of world.structures) {', "if (distance(person, structure) > CONSTRUCTION_RADIUS || tileAt(world, structure)?.terrain !== 'shelter') continue;"]),
  a('obra.personas', 'personas', 'entrada', 7, 'inventions.ts', 'constructionContext', ['world.people.filter(other => distance(person, other) <= CONSTRUCTION_RADIUS)']),
  ...
  a('obra.reparaciones', 'estructuras', 'entrada', 7, 'inventions.ts', 'usefulRepairs', ['return world.structures.flatMap(structure => {', "if (structure.condition >= REPAIR_CONDITION_LIMIT || distance(person, structure) > CONSTRUCTION_RADIUS || tileAt(world, structure)?.terrain !== 'shelter') return [];"]),
  ...
  a('invento.personas', 'personas', 'entrada', 4, 'inventions.ts', 'inventionContext', ['world.people.filter(p => distance(person, p) <= 4).reduce((sum, p) => sum + Math.max(0, p.inventory - 0.08), 0)']),
  ...
  a('invento.planosEstructuras', 'estructuras', 'entrada', 5, 'inventions.ts', 'knownBlueprints', ["for (const structure of world.structures) if (distance(person, structure) <= 5 && structure.condition > BROKEN_CONDITION && tileAt(world, structure)?.terrain === 'shelter') ids.add"]),
  a('invento.planosPersonas', 'personas', 'entrada', 3, 'inventions.ts', 'knownBlueprints', ['if (world.cooperationEnabled) for (const other of world.people) {', 'if (other !== person && distance(person, other) <= 3 && (person.bonds[other.id] ?? 0.2) >= 0.2 && person.culture.openness >= 0.15)']),
  ...
  a('kernel.propia', 'teselas', 'entrada', 0, 'ecosystem-kernel.ts', 'EcosystemKernel', ['const length = tiles.length, neighbors = previous.neighbors, lifeBefore = previous.life;',
...
export const ALCANCES_SERIALES: readonly AlcanceSerial[] = [
  ...
  s('accion.compartirReceptor', 'accion', 'personas', 'actor', 2, 'index.ts', 'share', ['world.people.filter(p => p.id !== donor.id && p.hunger > 0.27 && distance(p, donor) <= 2)']),
  s('accion.observadores', 'accion', 'personas', 'actor', 3, 'index.ts', 'share', ['for (const observer of world.people) {', 'distance(observer, donor) > 3']),
  ...
  s('accion.disputa', 'accion', 'personas', 'actor', { param: 'social.disputaRadio' }, 'society.ts', 'resourceDispute', ['world.people.find(p => p !== person && p.communityId && p.action === person.action && distance(person, p) <= disputaRadio && distance(person.target, p.target) < disputaDestino']),
  ...
  s('estructuras.deposito', 'estructuras', 'personas', 'actor', 1.5, 'inventions.ts', 'stepStructures', ['if (a.foodCapacity > 0) for (const person of world.people) {', 'distance(person, structure) > 1.5']),
  ...
  s('herencia', 'demografia', 'personas', 'actor', 2, 'index.ts', 'transferEstate', ['world.people.filter(other=>other!==person&&other.demography.deathCause===null&&distance(person,other)<=2)']),
  s('herencia.tecnologica', 'demografia', 'personas', 'actor', 2, 'technology.ts', 'settleTechnologyEstate', ['host.people.includes(p) && distance(actor, p) <= 2']),
  s('muerte.tesela', 'demografia', 'teselas', 'actor', 0, 'lineage.ts', 'advancePopulation', ['const tile = tileAt(world, person);']),
  s('muerte.techo', 'demografia', 'estructuras', 'actor', 0, 'lineage.ts', 'advancePopulation', ["tile?.terrain === 'shelter' ? Math.max(0, ...world.structures",
    '.filter(structure => structure.x === person.x && structure.y === person.y && structure.condition > BROKEN_CONDITION']),
  s('comunidad.alternativas', 'comunidades', 'personas', 'actor', 6, 'society.ts', 'updateCommunities', ['world.people.filter(p => p !== person && p.communityId !== group.id && distance(person, p) <= 6']),
  s('comunidad.vecinos', 'comunidades', 'personas', 'actor', 6, 'society.ts', 'updateCommunities', ['world.people.filter(p => p !== person && distance(person, p) <= 6 && (person.bonds[p.id] ?? 0) >= 0.25']),
  ...
  s('convivencia.vecinos', 'comunidades', 'personas', 'actor', 6, 'society.ts', 'reviseByCohabitation', ['world.people.filter(p => trustedNeighbor(person, p))'],
  ...
  s('reproduccion.pareja', 'reproduccion', 'personas', 'actor', { param: 'poblacion.radioPareja' }, 'index.ts', 'reproduce', ['world.people.filter(p => match(a, p))', 'distance(a, b) <= pop.radioPareja'],
...
export const LECTURAS_GLOBALES: readonly LecturaGlobal[] = [
  ...
  { id: 'planos.identidad', fase: 'accion', fuente: 'world.blueprints + world.structures + world.retiredChunks', cota: 'O(planos + estructuras + chunks retirados)', fichero: 'inventions.ts', funcion: 'blueprintIdentity',
    ...
    motivo: 'Id nuevo de plano que no repita ninguno vivo ni archivado.' },
  { id: 'estructuras.identidad', fase: 'accion', fuente: 'world.structures + world.retiredChunks', cota: 'O(estructuras + chunks retirados)', fichero: 'inventions.ts', funcion: 'completeConstruction',
    patrones: ["nextIdentity(world.structureCounter, 'structure', [...world.structures.map(s => s.id), ...world.retiredChunks.flatMap(chunk => (chunk.structures ?? []).map(s => s.id))]);"],
    motivo: 'Id nueva de estructura que no repita ninguna viva ni archivada.' },
  ...
  { id: 'fauna.ventana', fase: 'fauna', fuente: 'world.animals', cota: 'MAX_ACTIVE_ANIMALS = 8 192', fichero: 'animals.ts', funcion: 'stepAnimals',
    patrones: ['const offset = population ? ((world.tick % population) * MAX_ACTIVE_ANIMALS) % population : 0;', 'const selected = population <= MAX_ACTIVE_ANIMALS ? [...world.animals]',
    ...
    motivo: 'Con más de 8 192 animales, sólo vive este paso una ventana rotatoria del orden canónico de TODA la fauna activa: qué animales entran depende de cuántos hay en todo el mundo. Selección global: la hace el coordinador.' },
...
export const LECTURAS_POR_IDENTIDAD: readonly LecturaPorIdentidad[] = [
  ...
  { id: 'activacion.chunk', fase: 'activacion', fuente: 'world.chunks + world.retiredChunks', fichero: 'spatial.ts', funcion: 'activate',
    patrones: ['if (world.chunks[key]) return;', 'world.retiredChunks.findIndex(c => c.key === key)', 'world.chunks[key] = meta;'], motivo: 'Chunk por clave: ya activo, retirado o nuevo.' },
...
export const RECORRIDOS: readonly Recorrido[] = [
  ...
  { id: 'fauna.orden', fase: 'faunaSerial', fichero: 'animals.ts', funcion: 'stepAnimals', veces: 2, patrones: ['world.animals.sort(canonical);'], motivo: 'Orden canónico al empezar y al acabar.' },
  ...
  { id: 'demografia', fase: 'demografia', fichero: 'lineage.ts', funcion: 'advancePopulation',
    patrones: ['const transitions = world.people.map(person => {', 'world.people = world.people.filter(', 'for (const person of world.people) for (const id of departed)',
  ...
  { id: 'activacion.retiro', fase: 'activacion', fichero: 'spatial.ts', funcion: 'maintainRegions',
    patrones: ['for (const tile of world.tiles) {', 'for (const place of world.places) detached', 'delete world.chunks[key];', 'world.animals = world.animals.filter(', 'world.structures = world.structures.filter(', 'world.places = world.places.filter('],
