...
export function assertEcologyState(world: World): void {
  ...
  if (!state || state.version !== 1 || state.originTick !== 0 || !safe(state.revision) || !safe(state.committedRevision, state.revision)
    ...
    || !Array.isArray(state.pendingClimate) || state.pendingClimate.length > 2) fail();
...
export function enableContinuousEcology(world: World, options: { jobsPerPulse?: number; chunkTicksPerJob?: number } = {}): void {
  ...
  world.ecology = { version: 1, originTick: 0, revision: 0, committedRevision: 0, cursor: null,
    ...
    summary: { regions: 0, sumTicks: 0, oldestTick: null }, pending: [], pendingClimate: [{ tick: 0, weather: world.weather }] };
...
export function ecologyStateForCommit(state: EcologyState): EcologyState {
  return { ...state, committedRevision: state.revision, pending: [], pendingClimate: [] };
}
export function markEcologyCommitted(world: World): void {
  if (!world.ecology) return;
  world.ecology.committedRevision = world.ecology.revision; world.ecology.pending = []; world.ecology.pendingClimate = [];
}
