...
import { limitsOf } from './params.js';
...
export function updateCommunities(world: World, emit: Emit): void {
  ...
  for (const person of world.people) {
    ...
    // T100: el tope de fundación es `limites.comunidades` (default 8 = conducta de siempre).
    // Elevarlo es un CAMBIO DE CONDUCTA declarado, no una relajación de validación.
    if (free.length < 2 || world.communities.length >= limitsOf(world).comunidades || !world.places.some(place => distance(person, place) <= 7)) continue;
