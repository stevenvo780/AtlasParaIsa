...
test('backup CLI keeps an older service schema and source bytes unchanged',()=>{
  ...
  store.db.exec('DROP TABLE legacy; DROP TABLE ecology_headers; DROP TABLE ecology_heads; DROP TABLE ecology_previous; DROP TABLE ecology_climate; DROP TABLE ecology_identities; PRAGMA user_version=2;');store.close();
  try {
