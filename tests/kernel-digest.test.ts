 ...
 *
 * Si una tarea cambia una LEY a propósito, este control se actualiza EN EL MISMO commit que la ley,
 * citando la evidencia: nunca al revés. La huella del mundo corre con `HISTORICAL_PARAMS`; la del
 * kernel aislado fija su ley desde que se tomó.
 *
 * Para medir el COSTE del kernel (no lo mide este test): un kernel por proceso, porque compartir
 * proceso vuelve megamórficas sus cachés en línea y puede invertir el signo de la comparación, y la
 * misma configuración en los dos lados, alternados (un lado con `--cpu-prof` y otro sin él no se
 * comparan). Las cifras medidas (T048, `5163ea2`) quedan en el historial de git de este fichero.
 */
...
test('600 pasos conservan la trayectoria declarada de 2026-09-22; el kernel aislado conserva su ley', () => {
  ...
  // Huella del mundo con las leyes históricas: solo cambia con un cambio de ley deliberado.
