...
import { type ConLimpieza, filaInstantanea, laboratorio, reescribirInstantanea } from './lib/store.js';
...
const laboratory = (t: ConLimpieza) => laboratorio(t, 'atlas-params-test-');
...
test('los parámetros del mundo viajan en la instantánea y mandan al recargar', t => {
  ...
  try {
    ...
    assert.equal(filaInstantanea(reopened).digest, saved.digest, 'el dígeste sobrevive a la ida y vuelta');
...
test('los defaults actuales son explícitos y una instantánea legacy sin params recibe el modo histórico', t => {
  ...
  reescribirInstantanea(store, JSON.stringify(antigua));
  const reopened = new Store(path);
...
test('unos parámetros fuera de rango en la instantánea fallan cerrado en vez de colarse en el mundo', t => {
  ...
  store.save(world);
  const cuerpo = JSON.parse(filaInstantanea(store).body) as Record<string, unknown>;
  const params = structuredClone(cuerpo.params) as { agua: { cuencas: number } };
  params.agua.cuencas = 9;
  reescribirInstantanea(store, JSON.stringify({ ...cuerpo, params }));
  ...
  reescribirInstantanea(store, desconocido);
...
test('T102: parámetros tipados sobreviven a guardado, reinicio, clon y overrides del despliegue', t => {
  ...
  const saved = filaInstantanea(store), reopened = new Store(path);
  try {
    ...
    reopened.save(loaded);
    assert.equal(filaInstantanea(reopened).body, saved.body, 'roundtrip tipado conserva los bytes');
...
test('T102: snapshot params-v1 anterior completa sólo los campos nuevos con defaults', t => {
  ...
  reescribirInstantanea(store, JSON.stringify(saved));
  const reopened = new Store(path);
  try {
    const loaded = reopened.load()!.world;
    assert.deepEqual(paramsOf(loaded), parseParams('limites.aplicacion=historicos', params));
    reopened.save(loaded);
    const expanded = JSON.parse(filaInstantanea(reopened).body).params;
...
test('T102: checksum recalculado no legitima flags, arrays o señales corruptos del snapshot', t => {
  ...
  for (const [section, key, value] of [
    ...
    reescribirInstantanea(store, JSON.stringify(corrupted));
