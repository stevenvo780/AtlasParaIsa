...
test('cutaways reveal only selected material and leafless wood keeps deterministic non-leafy branches',{timeout:30_000},async()=>{
 ...
 try{
  ...
  const result=await p.evaluate(async structure=>{
   ...
   const w={version:5,tick:900,sequence:900,phase:'day',weather:'clear',day:1,width:20,height:16,originX:-10,originY:-8,tiles,people:[],animals:[],structures:[structure],places:[],events:[],memories:[]};const preserved=JSON.stringify(w);
