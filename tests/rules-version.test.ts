...
for (const previousVersion of [6, 7]) test(`V8 is explicit and reading valid V${previousVersion} preserves its history, stocks, traits and parameters`, () => {
  ...
  assert.equal(RULES_VERSION, 8);
  assert.equal(createWorld().version, 8);
  assert.equal(migrated.version, 8);
  assert.deepEqual(migrated, { ...before, version: 8 });
  assert.deepEqual(world, before);
  assert.deepEqual(paramsOf(migrated), paramsOf(world));
  assertWorld(migrated);
});

for (const previousVersion of [6, 7]) test(`a V${previousVersion} label cannot launder corrupt physical state through the V8 reader`, () => {
