...
function oldDatabase(path: string, version: 1 | 2 | 3, worlds: World[]) {
  ...
  try {
    ...
    store.db.exec('DROP TABLE ecology_headers; DROP TABLE ecology_heads; DROP TABLE ecology_previous; DROP TABLE ecology_climate; DROP TABLE ecology_identities;');
    if (version < 3) store.db.exec('DROP TABLE legacy');
    if (version < 2) store.db.exec('DROP TABLE chunks');
  } finally { store.close(); }
}

test('new stores install SQLite schema 5 and initial coverage zero in the first atomic snapshot', t => {
