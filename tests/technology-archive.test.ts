...
import { defaultTechnologyState, initialTechnologyKnowledge, programSignature, researchTechnology,
...
test('an existing pre-origin child is validated and cannot hide malformed evidence behind the boundary', t => {
  ...
  db.prepare("UPDATE technology_executions SET body=?,digest=? WHERE id='process-1'").run(body, sha256(body));
...
test('origin checksum, strict shape, identity and numeric bounds are verified without read-side mutations', t => {
  ...
  for (const value of [null, { version: 2, startsAfter: 1 }, { version: 1, startsAfter: -1 },
    ...
    db.exec('BEGIN'); const body = JSON.stringify(value); db.prepare('UPDATE technology_origin SET body=?,digest=?').run(body, sha256(body));
...
test('recomputed digests do not excuse malformed definitions, statistics or receipts, including key/tick disagreement', t => {
  ...
  const corrupt = (table: string, mutate: (value: any) => void, read: () => unknown) => {
    ...
    db.exec('BEGIN');
    try { db.prepare(`UPDATE ${table} SET body=?,digest=?`).run(body, sha256(body)); assert.throws(read, /Invalid technology archive/); }
...
test('queries reject visible statistical and receipt time regressions without consulting future statistics', t => {
  ...
  db.exec('BEGIN'); db.prepare('UPDATE technology_stats SET body=?,digest=? WHERE tick=20').run(body, sha256(body));
  ...
  db.exec('BEGIN'); db.prepare('UPDATE technology_executions SET tick=9,body=?,digest=? WHERE serial=2').run(receipt, sha256(receipt));
...
test('as-of reads never decode unrelated future rows, including invalid bodies with matching digests', t => {
  ...
  transaction(db, () => {
    for (const table of ['technology_definitions', 'technology_stats', 'technology_executions']) db.prepare(`UPDATE ${table} SET body=?,digest=? WHERE tick=20`).run('null', sha256('null'));
  });
...
test('truncate refuses a corrupt retained reference before deleting any future row', t => {
  ...
  db.exec('BEGIN'); db.prepare('UPDATE technology_definitions SET tick=10,body=?,digest=? WHERE id=?').run(body, sha256(body), child.id);
...
test('recovery can discard corrupt future statistics and receipts without consulting their bodies', t => {
  ...
  transaction(db, () => {
    db.prepare('UPDATE technology_stats SET body=?,digest=? WHERE tick=20').run('null', sha256('null'));
    db.prepare('UPDATE technology_executions SET body=?,digest=? WHERE tick=20').run('[]', sha256('[]'));
